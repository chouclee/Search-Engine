...
public class QryEval {
  ...
  public static void main(String[] args) throws Exception {
   ...
   // int N = READER.numDocs();
    ...
    ArrayList<ScoreList.ScoreListEntry[]> letorResult = new ArrayList<ScoreList.ScoreListEntry[]>();
    ArrayList<Integer> docNum = new ArrayList<Integer>();
    ...
    for (Integer queryID : queriesID) {
      ...
      /****************************Learn To Rank*******************************/
      if (letor != null) {
        QryResult qryRes = operation.evaluate(model);
        ScoreList.ScoreListEntry[] topRank = getTopNDocuments(qryRes , 100);
        letor.evaluate(topRank, params, queryID);
        letorResult.add(topRank);
        docNum.add(topRank.length);
      }
      else 
    ...
    if (letor != null) {
      ...
      // read score files
      scan = new Scanner(new File(params.get("letor:testingDocumentScores")));
      line = null;
      ...
      scan.close();
      int k = 0;
      Comparator<ScoreList.ScoreListEntry> reverseOrder = new ScoreList.ReverseScoreOrder();
      for (int i = 0; i < letorResult.size(); i++) {
        ScoreList.ScoreListEntry[] topRank = letorResult.get(i);
        for (int j = 0; j < topRank.length; j++) {
          topRank[j].setScore(scores.get(k++));
        }
        Arrays.sort(topRank, reverseOrder);
        writeTrecEvalFile(params.get("trecEvalOutputPath"), queriesID.get(i), topRank);
      }
    }
   ...
   *          Result object generated by {@link Qryop#evaluate()}.
   */
  static void writeTrecEvalFile(String filePath, int queryID, 
          QryResult result) {
  ...
  static void writeTrecEvalFile(String filePath, int queryID, 
          ScoreList.ScoreListEntry[] topRank) throws Exception {
  ...
  static ScoreList.ScoreListEntry[] getTopNDocuments(QryResult result, int numDocs) {
    ...
    PriorityQueue<ScoreList.ScoreListEntry> pq = new PriorityQueue<ScoreList.ScoreListEntry>(
            numDocs, SCORE_ORDER);
    int cnt = 0;
    numDocs = Math.min(numDocs, result.docScores.scores.size());
    for (; cnt < numDocs; cnt++) {
